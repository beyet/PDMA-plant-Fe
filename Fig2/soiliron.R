...
library(agricolae)
library(ggplot2)
app = read_excel("soiliron.xlsx",sheet = 1) 

bartlett.test(ironp~indexp, data = app) 
aovapp <- aov(ironp~indexp,data = app) 
summary(aovapp)
...
p1<-ggplot(hei_mean,aes(x=indexp,y=ironp))+
  ...
  ylab("Available iron in soil (mg·kg-1soil)")+  
  geom_text(aes(label=c("b","b","a","a"),y=ironp+hei_sd$ironp+0.5,family = "Times"),vjust=0)+
